...
def sub_load_data(data, img_size, aug):
    ...
    else: mnt = np.array(mnt_reader(dataset+'mnt_results/'+img_name+'.mnt'), dtype=float)
    ali = cv2.resize(ali, dsize=(img.shape[1], img.shape[0]))
    seg = cv2.resize(seg, dsize=(img.shape[1], img.shape[0]))
    print(mnt.shape)
    print(dataset)
    print(img_name)
    if any(img.shape != img_size):
        ...
        if any(img.shape > img_size):
            print(img_t.shape)
            tra = np.absolute(tra)
            img_t= img[tra[0]:img.shape[0]-tra[0],tra[1]:img.shape[1]-tra[1]]
            seg_t = seg[tra[0]:img.shape[0]-tra[0],tra[1]:img.shape[1]-tra[1]]
            ali_t = ali[tra[0]:img.shape[0]-tra[0],tra[1]:img.shape[1]-tra[1]]
            print(img_t.shape)
        else:
...
# for getting label: label_mnt_s, label_mnt_w, label_mnt_h, label_mnt_o
...
def get_maximum_img_size_and_names(dataset, sample_rate=None, max_size=None):
    ...
    set_n = dataset[0].split('/')[-3]
    data_ending = ".bmp"
    if set_n == 'FVC2004' or set_n == 'FVC2002':
        data_ending = '.tif'
    if set_n == 'FVC2006' or set_n == 'FVC2006_test':
        data_ending = '.bmp'
    if set_n == 'FVC2006_png' or set_n == 'FVC2006_test_png' or set_n == 'FVC2002_png':
        data_ending = '.png'
